...
from config import invoke_llm, get_database_url
...
def _truncate(text, max_chars: int = 600) -> str:
    """
    Shorten a previous agent's output before re-feeding it into the next
    prompt. Groq's free tier caps tokens-per-minute (TPM) per request, and
    this pipeline re-embeds every earlier result into each later prompt
    (budget -> itinerary -> final), so the context would otherwise keep
    growing until a request tips over the limit. The full, untruncated
    text is still returned to the frontend for the user to read - only the
    copy fed back into the next LLM call is shortened.

    Accepts anything, not just str: MCP tool results can come back as
    lists of content blocks rather than plain text, so this always
    normalizes to a string first instead of assuming the input is one.
    """
...
def flight_agent(state: TravelState):
    ...
    try:
        ...
        # AviationStack's free plan returns a normal (non-error) response
        # that itself says the endpoint isn't included in that plan - it
        # doesn't raise an exception, so the except block below never
        # triggers for this case. Detect it explicitly and fall back to
        # letting the model answer from general knowledge instead of
        # feeding it a raw "function_access_restricted" error string.
        aviation_unavailable = "function_access_restricted" in airports or (
            "function_access_restricted" in airlines
        )

        if aviation_unavailable:
            airport_data = (
                "Live airport data is unavailable on the current "
                "AviationStack plan. Use general knowledge instead."
            )
            airline_data = (
                "Live airline data is unavailable on the current "
                "AviationStack plan. Use general knowledge instead."
            )
        else:
...
Draft Itinerary:
{_truncate(state.get('itinerary', ''), max_chars=1500)}
